...
describe('/admin page access', () => {
  ...
  it('redirects non-admin users', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({
      ...
      user: { id: '1', name: 'User', email: 'u@b.c', role: 'member', createdAt: new Date(), updatedAt: new Date(), emailVerified: true, image: null }
    });
    const mod = await import('../../src/app/admin/page');
    await mod.default();
    expect(vi.mocked(redirect)).toHaveBeenCalledWith('/');
  ...
  it('allows admin users', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({
      session: { id: '1', userId: '1', token: 'token', expiresAt: new Date(), createdAt: new Date(), updatedAt: new Date() },
      user: { id: '1', name: 'Admin', email: 'a@b.c', role: 'admin', createdAt: new Date(), updatedAt: new Date(), emailVerified: true, image: null }
    });
    const mod = await import('../../src/app/admin/page');
  ...
  it('handles users without role', async () => {
    vi.mocked(auth.api.getSession).mockResolvedValue({
      session: { id: '1', userId: '1', token: 'token', expiresAt: new Date(), createdAt: new Date(), updatedAt: new Date() },
      user: { id: '1', name: 'User', email: 'u@b.c', createdAt: new Date(), updatedAt: new Date(), emailVerified: true, image: null, role: 'member' }
    });
